...
class EventScraper:
    ...
    async def _scrape_source(self, session: aiohttp.ClientSession, url: str) -> List[Event]:
        ...
        async with session.get(url) as response:
            ...
            # (Optional) Write prettified HTML to file
            ...
            if "eventbrite" in url:
                ...
                for element in soup.select(selector):
                    try:
                        ...
                        # 4) Image URL (if available)
                        img_tag = element.select_one("img")
                        image_url = img_tag["src"] if img_tag and img_tag.has_attr("src") else ""

                        # 5) Parse raw_date_time into a datetime (assume current year, handle “today”, “tomorrow”, weekdays)
                        date_obj = self._parse_date_time(raw_date_time)

                        # 6) Build Event object (date can be None if parsing failed)
                        event = Event(
                            title=title,
                            description="",
                            date=date_obj,
                        ...
                        # 7) Append to list and write to file as JSON Lines
                        events.append(event)

                        with open("scraped_events.jsonl", "a", encoding="utf-8") as fp:
                            # Exclude None fields and “id” before dumping
            ...
            # --- Add other sources (Moshtix, Ticketmaster) similarly ---
            # elif "moshtix" in url:
            #     ...
    ...
    def _parse_date_time(self, raw: str) -> Optional[datetime]:
        ...
        # Split on commas: last part is the time (e.g. "8:30 PM")
        parts = [p.strip() for p in raw.split(",")]
        time_part = parts[-1]  # e.g. "8:30 PM"

        # Attempt to parse the time portion
        try:
        ...
        # Helper: get next occurrence of a weekday (0=Monday ... 6=Sunday)
        ...
        # 2) “tomorrow, 8:30 PM”
            ...
            if first_part in weekdays_full:
                ...
                # 4) Attempt to parse a month/day format, e.g. “May 28” or “May 28 2025”
                # Combine the part(s) that likely contain a month name
                month_day_part = None
                for p in parts:
                    # Look for any of the month names or abbreviations
